...
import { arrayify } from '@ethersproject/bytes/lib';
 ...
 */
export function ecRecoverTypedSignature(hashOrMessage: string, typedSignature: string): Address {
  ...
  return recoverAddress(arrayify(prependedHash!), signature);
}
...
export function addressesAreEqual(addressOne: string, addressTwo: string): boolean {
